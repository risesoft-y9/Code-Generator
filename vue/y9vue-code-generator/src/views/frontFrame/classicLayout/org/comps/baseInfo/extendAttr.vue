<template>
    ...
    <y9Table :config="tableConfig">
        <template #name="{ row, column, index }">
            <el-input v-if="editId === index" v-model="formData.name" :size="fontSizeObj.buttonSize" />
            <template v-else>{{ row.name }}</template>
        </template>

        <template #attrValue="{ row, column, index }">
            <el-input v-if="editId === index" v-model="formData.attrValue" :size="fontSizeObj.buttonSize" />
...
<script lang="ts" setup>
    ...
    import { savePersonExtendProperties } from '@/api/frontFrame/person/index';
    import { saveOrgExtendProperties } from '@/api/frontFrame/org/index';
                        ...
                        let saveActions = $tableHandleRender([
                            {
                                ...
                                onClick: async () => {
                                    if (!formData.value.name) {
                                        ...
                                        if (props.currInfo.orgType == 'Person') {
                                            loading.value = true;
