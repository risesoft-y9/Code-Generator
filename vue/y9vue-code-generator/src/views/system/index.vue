...
<template>
        ...
        <template #rightContainer>
            <template v-if="isShow">
                ...
                <y9Card :title="`${$t('实体列表')}${currTreeNodeInfo.name ? ' - ' + currTreeNodeInfo.name : ''}`">
                    <div
                        style="display: flex; justify-content: space-between; text-align: right; margin-top: -10px"
                        class="expand-btns"
                    >
                    </div>
                    <entityList :systemId="currTreeNodeInfo.id" :treeNodeName="currTreeNodeInfo.name"> </entityList>
                </y9Card>
            </template>
            <template v-else>
    ...
    <y9Dialog v-model:config="dialogConfig">
        ...
        <y9Table
            v-if="dialogConfig.type === 'addTemplate'"
